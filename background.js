 ...
 */
async function startHarvest(apiKey, cseId, domains) {
  ...
  for (const domain of domains) {
    ...
    try {
      ...
      items.forEach((item) => categoriseAndStore(item.link, currentState));
      ...
      currentState.status = `Fetched ${domain} page 1 of ${pagesCount}`;
      persistState(currentState);
    } catch (err) {
      // Regardless of error type, mark harvest as no longer running to prevent the
      // UI from lingering in a "Starting…" state.  Propagate a descriptive
      // status message to the popup so the user knows what happened.
      currentState.running = false;
      if (err && err.reason === 'dailyLimitExceeded') {
        currentState.status = 'Stopped: Google Custom Search daily quota exceeded. Please retry after reset.';
      } else if (err && (err.reason === 'rateLimitExceeded' || String(err.message).toLowerCase().includes('rate limit'))) {
        currentState.status = 'Rate limit exceeded. Please try again later.';
      } else {
        currentState.status = `Error fetching ${domain} pages: ${err.message}`;
      }
      persistState(currentState);
      return;
    }
    ...
    try {
      ...
      currentState.completedSteps++;
      currentState.status = `Fetched ${domain} images page 1 of ${imagePages}`;
      persistState(currentState);
    } catch (err) {
      // Similarly handle image fetch errors: set running false and update status.
      currentState.running = false;
      if (err && err.reason === 'dailyLimitExceeded') {
        currentState.status = 'Stopped: Google Custom Search daily quota exceeded. Please retry after reset.';
      } else if (err && (err.reason === 'rateLimitExceeded' || String(err.message).toLowerCase().includes('rate limit'))) {
        ...
        currentState.status = `Error fetching ${domain} images: ${err.message}`;
      }
      persistState(currentState);
      return;
    }
    ...
    currentState.totalSteps += pagesCount + imagePages;
  }
  ...
  for (const task of tasks) {
    ...
    for (let pageNum = 2; pageNum <= pagesCount; pageNum++) {
      currentState.status = `Fetching ${domain} pages (${pageNum}/${pagesCount})`;
      persistState(currentState);
      try {
        const data = await fetchWithBackoff(apiKey, cseId, domain, 1 + (pageNum - 1) * 10, false);
        (data.items || []).forEach((item) => categoriseAndStore(item.link, currentState));
      } catch (err) {
        ...
        currentState.status = `Error on ${domain} page ${pageNum}: ${err.message}`;
        persistState(currentState);
      }
      currentState.completedSteps++;
      persistState(currentState);
    ...
    for (let pageNum = 2; pageNum <= imagePages; pageNum++) {
      currentState.status = `Fetching ${domain} images (${pageNum}/${imagePages})`;
      persistState(currentState);
      try {
        ...
        currentState.status = `Error on ${domain} image page ${pageNum}: ${err.message}`;
        persistState(currentState);
      }
      currentState.completedSteps++;
      persistState(currentState);
  ...
  // Mark finished
  currentState.running = false;
  currentState.status = `Fetched ${currentState.pagesResults.length + currentState.assetsResults.length} URLs.`;
  persistState(currentState);
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === 'startFetch') {
    ...
    // Reset any existing state and kick off new harvest
    startHarvest(apiKey, cseId, domains);
